...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(role === 'COACH' ? '/coach/dashboard' : '/client/home')
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#0B0D10]">
      <div className="w-full max-w-md p-8">
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 py-2 rounded-lg font-medium transition-colors ${
                  role === 'COACH'
                    ? 'bg-gold text-[#0B0D10]'
                    : 'bg-[rgba(255,255,255,0.04)] border border-[rgba(201,162,75,0.18)] text-text hover:border-gold'
                }`}
              >
                Coach
              </button>
        ...
        <p className="text-center text-text-muted text-sm mt-6">
          ...
          <Link href="/login" className="text-gold hover:text-[#E3C77E]">
            Sign In
          </Link>
        </p>
      </div>
